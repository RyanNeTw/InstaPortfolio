...
function Header(props) {
  ...
  return (
    <>
      <RateLimitAlert />
      {props?.user?.data?.hireable && hireMe ? (
        <HireMeModal user={props.user.data} />
      ) : null}
      <nav className="absolute bottom-0 border-t w-screen border-white flex flex-row justify-between pl-8 pr-8 pt-2 pb-2 bg-black ">
        <ul className="flex flex-row justify-center gap-4">
          <li className="rounded transition">
            <NavLink
              ...
              className={({ isActive }) =>
                isActive
                  ? "bg-zinc-600 flex flex-row items-center gap-2 text-white p-4 rounded"
                  : "hover:bg-zinc-600 rounded flex flex-row items-center gap-2 text-white p-4"
              }
            >
              <HomeSvg />
        ...
        <ul className="flex flex-row items-center gap-4">
          <span
            className="hover:opacity-70 cursor-pointer hidden md:block"
