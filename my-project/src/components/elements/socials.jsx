...
import { GetSocials } from "../../api/GetAccountInfo"
import GitHubSvg from "../../assets/GitHub"
import LinkedInSvg from "../../assets/LinkedIn"
import WorldSvg from '../../assets/world'

function SocialsList() {

    const [socials, setSocials] = useState([])
    useEffect(()=> {
        GetSocials().then((data) => setSocials(data.data))
    },[])

    console.log(socials)

    function getSvgForProvider (provider) {
        switch (provider) {
            case "linkedin" :
                return (<LinkedInSvg />)
            case "github": 
    ...
    if(!socials) {
        return(
            <div className="relative items-center block max-w-md bg-zinc-600 border border-zinc-600 rounded-lg shadow-md">
        ...
        <div className="flex flex-row gap-2 items-center">
            {
                socials.map((social, index) => {
                    const svg = getSvgForProvider(social.provider)
                    return (
                        <div key={index}>
