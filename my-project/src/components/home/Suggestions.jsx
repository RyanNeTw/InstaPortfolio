...
function Suggestions(props) {
  ...
  return (
    <>
      {modalState ? (
        <ContactModal setAction={setModalSate} action={modalState} />
      ) : null}
      <div className="pt-2 md:pt-8 flex flex-col gap-4 pl-4 pr-4 pb-3">
        ...
        <div className="flex flex-row items-center justify-between gap-16">
          ...
          <h3
            className="text-yellow-700 font-bold text-xs cursor-pointer hover:text-yellow-500 whitespace-nowrap"
            onClick={() => onpenModal()}
          >
            Contact me
          </h3>
        </div>
        <h3 className="text-white opacity-70 text-sm font-bold">
          Suggested for you
        </h3>
          ...
          {followings && followings?.length > 0
            ? followings.map((following, index) => {
                return (
                  <div
                    key={index}
                    className="flex flex-row items-center justify-between gap-2 md:gap-16"
                  >
                    <Link
                      to={`/profil/${following.login}`}
                    ...
                    >
                      ...
                      <div>
                        <h3 className="text-white text-sm font-bold uppercase">
                          {following.login.substring(0, 10)}
                        </h3>
                      </div>
                    </Link>
                    <a
                      className="text-yellow-700 font-bold text-xs cursor-pointer hover:text-yellow-500 hidden md:block"
